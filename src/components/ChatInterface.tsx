...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ btcData, embedded = false, onOpenAuth, onOpenUpgrade }) => {
  ...
  // There's no floating trigger for this panel anymore — the "AI Chat" nav
  // item (App.tsx) opens it via this event instead, same cross-component
  // pattern as open-upgrade-modal.
  useEffect(() => {
    const onOpenAiChat = () => {
      if (!hasPro) {
    ...
    return () => window.removeEventListener("open-ai-chat", onOpenAiChat);
  }, [hasPro, user, onOpenAuth, onOpenUpgrade]);
  ...
  // No floating trigger anymore, on either platform — the "AI Chat" nav
  // item (App.tsx) opens this via the open-ai-chat event instead. Mobile
  // web still has no entry point to this panel at all.
