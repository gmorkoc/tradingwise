    ...
    <div className="coin-chat-composer">
      {!user ? (
        ...
        <div className="coin-chat-composer-inner">
          ...
          <button
            type="button"
            className="coin-chat-send"
            onClick={handlePost}
            disabled={posting || uploadingImage || (!draft.trim() && !pendingImage)}
            aria-label={t("coinChat.post")}
          >
            {uploadingImage ? (
              <span className="coin-chat-send-spinner" />
            ) : draft.trim() || pendingImage ? (
              // Has content to send — same up-arrow ChatGPT switches to
              // once you start typing.
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round">
                <path d="M12 19V5M5 12l7-7 7 7" />
              </svg>
            ) : (
              // Empty composer — same waveform glyph ChatGPT's own send
              // button shows before you've typed anything (purely visual
              // parity here; this button has no voice-input mode, it's
              // just disabled in this state like before).
