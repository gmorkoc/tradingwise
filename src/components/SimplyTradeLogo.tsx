...
export const CoinHintzLogo: React.FC<Props> = ({ loading = false }) => (
  <span className={`st-logo${loading ? " st-logo--wave" : ""}`}>
    ...
    >
      <defs>
        <linearGradient id="tw-bg" x1="0" y1="0" x2="36" y2="36" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#4f46e5" />
          <stop offset="100%" stopColor="#0284c7" />
        </linearGradient>
      </defs>
      <polyline points="5,9 11,27 18,13 25,27 31,9" stroke="url(#tw-bg)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" fill="none" />
    ...
    <span className="st-logo-wordmark">
      ...
      {WORD2.split("").map((ch, i) => (
        <span
          ...
          style={loading ? { animationDelay: `${(WORD1.length + i) * 0.06}s` } : undefined}
        >
          {ch}
        </span>
      ))}
